...
interface EditCommonDataProps {
  type: string;
  data: TypeData[];
  setState(key: string, value: any): void;
}

export function EditCommonData({ type, data, setState }: EditCommonDataProps) {
  return (
    <>
      {type === "CPF" && (
        <Grid item xs={12} lg={2}>
          <TextField
            value={data[8].value ? data[8].value : ""}
            size="small"
      ...
      <Grid item xs={12} lg={12}>
        <TextField
          ...
          variant="outlined"
          fullWidth
          onChange={(e) => setState(data[7].fieldName, e.target.value)}
        />
      </Grid>
    </>
  );
